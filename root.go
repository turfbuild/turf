...
func newRootCmd() *cobra.Command {
	...
	cmd.PersistentFlags().StringVar(&flagModel, "model", os.Getenv("TURF_MODEL"), "LLM model: a named model from turf.yaml, an inline provider/model (e.g. anthropic/claude-sonnet-4-6, keyless dmr/ai/qwen3), or 'auto' to pick the first provider with credentials. Default: turf.yaml model:, else auto (env: TURF_MODEL)")
