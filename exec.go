...
func newExecCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Run one natural-language request against the model (set with --model) without launching the interactive TUI, " +
			...
			"The request comes from the [message] argument, from stdin when [message] is \"-\", or from piped stdin when no " +
			"argument is given (with a terminal and no argument, a plain readline prompt loop starts instead).\n\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// The caller's message(s) go straight to cli.Run: a single arg (or
			// "-" for stdin) runs one turn; no arg lets cli.Run read piped stdin
			// or start a readline loop on a TTY. The session carries no seeded
			// message — the prompt IS the user turn, so it is not double-sent.
